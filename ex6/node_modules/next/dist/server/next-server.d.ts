...
import type { Revalidate, ExpireTime } from 'next/dist/server/lib/revalidate';
...
import ResponseCache, { type IncrementalCacheItem } from 'next/dist/server/response-cache';
...
export default class NextNodeServer extends BaseServer<Options, NodeNextRequest, NodeNextResponse> {
    ...
    protected imageOptimizer(req: NodeNextRequest, res: NodeNextResponse, paramsResult: import('next/dist/server/image-optimizer').ImageParamsResult, previousCacheEntry?: IncrementalCacheItem): Promise<{
        buffer: Buffer;
        contentType: string;
        maxAge: number;
        upstreamEtag: string;
